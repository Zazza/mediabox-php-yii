define(function (require) {
    ...
    var $ = require('jquery');
    var MediaboxFunctions = require('mediabox/app/mediabox/mediabox-functions');
        ...
        this.getTagsAndCrops = function() {
            $.ajax({ type: "GET", url: '/image/getAllTags/', dataType: "JSON" })
                .done(function(res) {
                    ...
                    $.each(res, function(key, value) {
                        $(".left-section #allTags").append("<span class='tagAll k-content'>" + value.tag + "</span> ");
                    });
                    ...
                    $.ajax({ type: "GET", url: '/image/getAllCrops/', dataType: "JSON" })
                        .done(function(res) {
                            $.each(res, function(key, value) {
                                $(".left-section #allCrops").append("<span class='cropAll k-content'>" + value.description + "</span> ");
                            });
                        })
                });
        }
